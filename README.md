...
* **Script2_extractPitch.ipynb** : extracts the fundamental frequency (f0), harmonic-to-noise (HNR) and root mean square (RMS) of the vocalisations in each phase of the experiment (experimental and baseline) with Praat, based on the .csv file generated by Script1_cutFiles.ipynb. To run this script, download **Pitch_BB.praat** and **utilities_SOA.py** (both included in this repository). Make sure to update your local path to Pitch_BB.praat inside utilities_SOA.py. This script generates a .csv file with the following name structure: "f0_HNR_RMS_NX.csv". 
* **Script3_plotResults.ipynb** : preprocesses data, plots results and runs descriptive statistics based on the results file with the following format: "f0_HNR_RMS_NX.csv". It also generates 4 different scripts for subsequent statistical analysis in R. These have the following format and overall content: "NX_B.csv" (containing the standardised f0 difference between experimental and baseline phase per individual vocalisation), "NX_both.csv" (containing also the caregiver' vocalisations), "NX_concat.csv" (containing the standardised difference in the f0 produced between phases calculated over concatenated data, employed in the main analysis in the manuscript),"NX_concat_both.csv" (containing the standardised difference in the f0 produced between phases calculated over concatenated data for both the infant an the caregiver), "N72_phases.csv" (the average difference between the unmodified and modified sound in each phase, per infant). "X" is the participant number (48 for the experimental groups and 72 overall, including the neutral group).
* **Script4_f0overtime.ipynb** allows to visualise f0 variation over time, overall and per vocalisation. NOTE: the dataset this code operates on is too large to be uploaded in GitHub.

Please note that Script 1 and Script 2 are provided for illustrative purposes only, to demonstrate our processing steps. They cannot be executed on the data in this repository, as they require access to the original audio recordings, which cannot be shared publicly for privacy reasons.
